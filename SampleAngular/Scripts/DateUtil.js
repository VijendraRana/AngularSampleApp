...
var DateUtil = {
    ...
    /*
        Formats date obtained from server (MS .NET JSON Date, eg. "/Date(1238540400000)/") to specified format ("general" if none specified)
        Also formats any other valid date format.
    ...
    */
    convertToJSDate: function (date, format) {
        ...
        // Obtain milliseconds
        if (date) {
            // Regex detects and converts MS JSON date.
            var match = String(date).match(/^\s*\/?Date\((\d+)\)\/?\s*$/i); // ignore case, outer slashes optional, and leading/training whitespace allowed
            ...
            // Formatting applied to any valid date.
            if (!isNaN(jsDate)) {
    ...
    isDate: function (val) {
        ...
        return !isNaN(d) && d >= new Date("1901-01-01").valueOf() && d <= new Date("9999-12-12").valueOf();
    },
    isTime: function (val) {
        return (/^\s*\d{1,2}(:|\.)?\d{0,2}\s*(am|pm)?\s*$/i).test(val); // returns true or false based on a rather loose definition of a time
    },
...
if (this.JSON) {
    ...
    if (!this.JSON.dateStringToDate) {
        JSON.dateStringToDate = function (dtString, format) {
            /// <summary>
